...
// relógio alinhado ao servidor (corrige fuso/defasagem)
let TIME_SKEW = 0;                   
...
function ensureEtaMs(order) {
  if (!order.etaMs || order.etaMs <= 0) {
    ...
    const estMin = Math.min(30, 8 + totalQty * 2); // fallback
...
function render() {
  ...
  // preparar primeiro, depois aguardando; dentro do grupo, por seq
  const weight = (s) => (s === 'preparando' ? 0 : s === 'aguardando' ? 1 : 2);
  const active = [...activeOrders].sort((a, b) => (weight(a.status) - weight(b.status)) || ((a.seq || 0) - (b.seq || 0)));
  ...
  hist.innerHTML = '';
  const histSorted = [...historyOrders].sort((a, b) => (b.finalizedAt || b.createdAt) - (a.finalizedAt || a.createdAt));
  histSorted.forEach(o => {
    const row = document.createElement('div');
    row.className = 'history-item fade-in';
    const summary = o.items.map(it => `${it.qty}× ${it.name}`).join(' • ');
    row.innerHTML = `
      <div><strong>${o.code || ('#' + o.id)}</strong><div class="meta">Ord. ${String(o.seq || '-')} • ${brTime(o.createdAt, false)}</div></div>
...
function tickUI() {
  activeOrders.forEach(o => {
    ...
    // tempo decorrido no topo direito
    ...
    // preparando: barra + restante + decorrido
...
async function loadFeed() {
  try {
    ...
    // 1) Alinha relógio do cliente ao horário do servidor
    const srvNowTs = Date.parse(data.server_time);
    if (Number.isFinite(srvNowTs)) {
      TIME_SKEW = srvNowTs - Date.now();
    }

    // 2) Parser de datas no fuso do servidor (ex.: "2025-08-25 19:55:42")
    const srvTZ = (String(data.server_time).match(/([+-]\d{2}:\d{2})$/) || [])[1] || 'Z';
    const parseSrv = (s) => {
      if (!s) return NaN;
      const str = String(s).replace(' ', 'T');
      const hasTZ = /Z|[+-]\d{2}:\d{2}$/.test(str);
      const ts = Date.parse(hasTZ ? str : (str + srvTZ));
      return Number.isFinite(ts) ? ts : new Date(s).getTime();
    };

    //  Ativos (preparando/aguardando)
    activeOrders = (data.active || []).map(o => {
      const created = parseSrv(o.created_at);
      const last    = parseSrv(o.last_change || o.created_at);
      const started = (o.status === 'preparando' && o.prep_started_at)
        ? parseSrv(o.prep_started_at)
        : (o.status === 'preparando' ? last : null);
      ...
      return {
        ...
        // eta_min (minutos) vindo do back-end → milissegundos
        etaMs: Math.max(0, Math.round((Number(o.eta_min) || 0) * 60000))
      };
    });

    // 4) Histórico (últimas 24h)
...
function openConfirm(id) {
  ...
  const m = byId('confirmModal'); m.classList.add('show'); m.setAttribute('aria-hidden', 'false');
}
function closeConfirm() {
  const m = byId('confirmModal'); m.classList.remove('show'); m.setAttribute('aria-hidden', 'true'); pendingId = null;
}
